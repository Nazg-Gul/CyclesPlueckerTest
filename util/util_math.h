...
__forceinline const __m128 shuffle(const __m128& b)
{
	return _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(b), _MM_SHUFFLE(index_3, index_2, index_1, index_0)));
}

template<> __forceinline const __m128 shuffle<0, 0, 2, 2>(const __m128& b)
{
	return _mm_moveldup_ps(b);
}

template<> __forceinline const __m128 shuffle<1, 1, 3, 3>(const __m128& b)
{
	return _mm_movehdup_ps(b);
}

template<> __forceinline const __m128 shuffle<0, 1, 0, 1>(const __m128& b)
{
	return _mm_castpd_ps(_mm_movedup_pd(_mm_castps_pd(b)));
}

template<size_t index_0, size_t index_1, size_t index_2, size_t index_3> __forceinline const float3 shuffle(const float3& b)
{
	return _mm_castsi128_ps(_mm_shuffle_epi32(_mm_castps_si128(b), _MM_SHUFFLE(index_3, index_2, index_1, index_0)));
}

#ifndef __KERNEL_SSE3__
template<> __forceinline const float3 shuffle<0, 0, 2, 2>(const float3& b)
{
	return _mm_moveldup_ps(b);
}

template<> __forceinline const float3 shuffle<1, 1, 3, 3>(const float3& b)
{
	return _mm_movehdup_ps(b);
}
#endif

template<> __forceinline const float3 shuffle<0, 1, 0, 1>(const float3& b)
{
	return _mm_castpd_ps(_mm_movedup_pd(_mm_castps_pd(b)));
}
#endif
