                                                   ...
                                                   int prim_addr)
{
	...
	/* Calculate vertices relative to ray origin. */
	const float3 v0 = float4_to_float3(tri_c) - P;
	const float3 v1 = float4_to_float3(tri_a) - P;
	const float3 v2 = float4_to_float3(tri_b) - P;
	...
	const ssef dirX(dir.x, dir.x, dir.x, dir.x);
	const ssef dirY(dir.y, dir.y, dir.y, dir.y);
	const ssef dirZ(dir.z, dir.z, dir.z, dir.z);
	const ssef UVWW = crossX*dirX + crossY*dirY + crossZ*dirZ;
	const float minUVW = vreduce_min(UVWW)[0];
	const float maxUVW = vreduce_max(UVWW)[0];
#else  /* __KERNEL_SSE2__ */
	const float U = dot(cross(v2 + v0, e0), dir);
	const float V = dot(cross(v0 + v1, e1), dir);
	const float W = dot(cross(v1 + v2, e2), dir);
	...
	if((sign_T < 0.0f) ||
	   (sign_T > isect->t * xor_signmask(den, sign_den)))
	{
		return false;
	}
...
#endif
	{
		...
		isect->type = PRIMITIVE_TRIANGLE;
#ifdef __KERNEL_SSE2__
		isect->u = UVWW[0] * inv_den;
		isect->v = UVWW[1] * inv_den;
#else
		isect->u = U * inv_den;
		isect->v = V * inv_den;
#endif
